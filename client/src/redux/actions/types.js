// Library Reducer
export const IMPORT_SC_LIKES = "IMPORT_SC";
export const IMPORT_SONG = "IMPORT_SONG";
export const SET_QUERY = "SET_QUERY";
export const RESET_QUERY = "RESET_QUERY";
export const TOGGLE_NEW_PLAYLIST_FORM = "TOGGLE_NEW_PLAYLIST_FORM";
export const TOGGLE_ADD_PLAYLIST_FORM = "TOGGLE_ADD_PLAYLIST_FORM";
export const TOGGLE_DELETE_TRACK_FORM = "TOGGLE_DELETE_TRACK_FORM";
export const ADD_TO_PLAYLISTS = "ADD_TO_PLAYLISTS";
export const SET_NEW_PLAYLIST_NAME = "SET_NEW_PLAYLIST_NAME";
export const CREATE_NEW_PLAYLIST = "CREATE_NEW_PLAYLIST";
export const TOGGLE_EDIT_TRACK_FORM = "TOGGLE_EDIT_TRACK_FORM";
export const EDIT_TRACK = "EDIT_TRACK";
export const DELETE_TRACK = "DELETE_TRACK";

// User Reducer
export const PUSH_LIB_ROUTE = "PUSH_LIB_ROUTE";
export const SET_SC_USER = "SET_SC_USER";

// Music Player Reducer
export const PLAY = "PLAY";
export const PAUSE = "PAUSE";
export const SEEK = "SEEK";
export const TOGGLE_EXPANDED_PLAYER = "TOGGLE_EXPANDED_PLAYER";
export const SET_IS_LOADED = "SET_IS_LOADED";
export const SET_PLAYER = "SET_PLAYER";
export const SET_TRACK = "SET_TRACK";
export const SET_DURATION = "SET_DURATION";
export const SET_SEEK = "SET_SEEK";
export const NEXT_TRACK = "NEXT_TRACK";
export const PREV_TRACK = "PREV_TRACK";
export const ADD_TO_QUEUE = "ADD_TO_QUEUE";
export const REMOVE_FROM_QUEUE = "REMOVE_FROM_QUEUE";
export const PLAY_FROM_QUEUE = "PLAY_FROM_QUEUE";
export const SET_QUEUE = "SET_QUEUE";

// Search Reducer
export const SEARCH = "SEARCH";
export const SEARCH_ARTISTS = "SEARCH_ARTISTS";
export const LOAD_MORE_RESULTS = "LOAD_MORE_RESULTS";
export const SET_NEXT_HREF = "SET_NEXT_HREF";
export const ADD_TO_SEARCH_HISTORY = "ADD_TO_SEARCH_HISTORY";
